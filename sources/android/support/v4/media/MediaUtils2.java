...
class MediaUtils2 {
    ...
    static List<MediaItem2> convertToMediaItem2List(Parcelable[] itemParcelableList) {
        ...
        if (itemParcelableList != null) {
            for (int i = 0; i < itemParcelableList.length; i++) {
                if ((itemParcelableList[i] instanceof Bundle) && (item = MediaItem2.fromBundle(itemParcelableList[i])) != null) {
                    playlist.add(item);
                }
            }
        }
    ...
    static List<MediaSession2.CommandButton> convertToCommandButtonList(Parcelable[] list) {
        ...
        for (int i = 0; i < list.length; i++) {
            if ((list[i] instanceof Bundle) && (button = MediaSession2.CommandButton.fromBundle(list[i])) != null) {
                layout.add(button);
            }
        }
        return layout;
