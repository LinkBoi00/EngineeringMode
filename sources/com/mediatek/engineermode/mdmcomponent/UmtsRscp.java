...
class UmtsRscp extends CurveComponent {
    ...
    public void update(String name, Object msg) {
        ...
        if (str.equals(MDMContent.MSG_ID_FDD_EM_CSCE_SERV_CELL_S_STATUS_IND)) {
            float rscp = ((float) getFieldValue(data, "serv_cell." + "rscp", true)) / 4096.0f;
            float ecno = ((float) getFieldValue(data, "serv_cell." + "ec_no", true)) / 4096.0f;
            if (rscp > -120.0f && ecno > -25.0f) {
                addData(0, rscp);
            }
            ...
            if (operation == 1 && rat == 0) {
                ...
                int i = 0;
                while (i < num && i < 16) {
                    float rscp2 = ((float) getFieldValue(data, "choice.neigh_cells[" + i + "]." + "rscp", true)) / 4096.0f;
                    float ecno2 = ((float) getFieldValue(data, "choice.neigh_cells[" + i + "]." + "ec_no", true)) / 4096.0f;
                    if (rscp2 > -120.0f) {
                        c = 0;
                        if (ecno2 > -25.0f) {
                            addData(1, rscp2);
            ...
            while (i2 < num2 && i2 < 32) {
                ...
                int rscp3 = getFieldValue(data, "umts_cell_list[" + i2 + "]." + "RSCP", true) / 4096;
                if (cellType == 0) {
                    addData(0, (float) rscp3);
                } else if (cellType == 1 || cellType == 3) {
                    addData(1, (float) rscp3);
                } else if (cellType == 2) {
                    addData(2, (float) rscp3);
                }
                i2++;
            }
        }
